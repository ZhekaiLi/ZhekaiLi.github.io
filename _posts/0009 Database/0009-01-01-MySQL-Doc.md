...
对列、数据类型的进一步修饰：
- `primary key (c1)` 
将 c1 设置为<span style="background-color: yellow; color: black;">主键</span>，每个表的主键唯一；
主键值必须唯一标识表中的每一行，且不能为 NULL
- `zerofill` 
补0，仅更改显示效果；
例如对于 int(3) zerofill 输入1，则会显示001
- `not NULL` 表明任意行数据在该列的值都不能为空
- `auto_increment` 
表示键值顺序递增；
可修饰在 not NULL 之后，这样插入新数据时即使没有给该列赋值，也能成功创建，这条新数据的该列值 = max(该列已有值) + 1
...
```
<span style="background-color: yellow; color: black;">注意事项</span>
...
表示从属关系：
- `col1 in (select col2 from table2)`

比较字符串：
- `col_name like '%soft%'` 选取包含 ’soft‘ 字段的元素，例如 'banksoft.'
- `col_name like 'soft%'` 选取以 'soft' 字段结尾的元素
- `col_name like 's_ft%'` 下划线可以表示任意字符，多个则表示多个任意字符
...
### 4.3 子查询 Subquery
嵌套在 `FROM`, `WHERE` or `HAVING` 中，以解决query依赖问题
值判断: =, >, <, <>(不等号), ...
...
select count(col1 > 0 or null);  -- 特定列数值大于0的数据个数
```
**去重复**
```sql
-- 查看5号商品有几种不同的售价
...
`DATE_FORMAT() format` 
<details><summary>点击展开详细列表</summary>
...
<!----------------------------------------------------------->
## 11. Practice
[link](../Z-attach/0000-01-01-Practice.md)
