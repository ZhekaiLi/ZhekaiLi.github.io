---
layout: post
title: 一文搞定（三） —— 高斯过程回归 Gaussian Process Regression
...
## 1.1 多元高斯分布
$$\tag{1} p(\bm{x})=\prod_{i=1}^n p(x_i)=\frac{1}{(2\pi)^{\frac{n}{2}}\sigma_1...\sigma_n}\exp(-\frac{1}{2}[\frac{(x_1-\mu_1)^2}{\sigma_1^2}...\frac{(x_n-\mu_n)^2}{\sigma_2^2}])$$
进一步的，令 
1. $\bm{x-\mu}=[x_1-\mu_1,...,x_n-\mu_n]^T$
2. $\bm\Sigma=\begin{bmatrix}
   \sigma_1^2 & 0 & ... & 0 \\
   0 & \sigma_2^2 & ... & 0 \\
   ... & ... & ... & .... \\
   0 & ... & 0 & \sigma_n^2
...
$$\tag{2} p(\bm{x})=(2\pi)^{-\frac{n}{2}}|\bm\Sigma|^{-\frac{1}{2}}\exp(-\frac{1}{2}(\bm{x}-\bm{\mu})^T|\bm\Sigma|^{-1}(\bm{x}-\bm{\mu}) )$$

（留意上式中的 $(\bm{x}-\bm{\mu})^T|\bm\Sigma|^{-1}(\bm{x}-\bm{\mu})$，它与下文将要介绍的 **kernel function** 有着类似的形式）
## 1.2 高斯过程 Gaussain Process 
高斯过程是定义在连续域上的无限多个高维随机变量所组成的随机过程，可以看做是一个无限维的高斯分布。在作者的理解中，==通过高斯过程，我们能够将**离散的点分布**转化为**函数的分布**==。（当然，这些离散点本身需要满足高斯分布）

借用[参考资料 4](https://zhuanlan.zhihu.com/p/75589452) 中的插图，高斯过程在下图中的体现就是将红色的离散点变成蓝色的函数曲线（==该曲线不仅能够储存原离散数据集的部分特征，还能用于预测==）：
...
对于随机变量 $\{\zeta_t\}_{t\in T}$ （$T$ 可以是连续的时间或空间），如果对于任意 $n\in \N^+,\;\;t_1,...,t_n\to \zeta_1,...,\zeta_n$, 都有 $\bm{\zeta}=(\zeta_1,...,\zeta_n)^T \sim N(\bm{\mu},\bm{\Sigma})$, 那么 $\{\zeta_t\}_{t\in T}$ 就是一个高斯过程，记做 
...
$P(y^*|Data,X^*)=\int_wP(y^*|w,Data,X^*)P(w)dw$
2. function-space view: 关注的是 $f(x)$
$P(y^*|Data,X^*)=\int_wP(y^*|f,Data,X^*)P(f)df$
# 2 Weight-space view
回顾贝叶斯线性回归（见[参考资料 2](https://blog.csdn.net/weixin_43728138/article/details/108377184)）
1. Inference: 求后验 $P(w|Data)=N(w|\mu_w,\Sigma_w)$
where, $\begin{cases}
...
$P(f(x^*)|Data,x^*)=N((x^*)^T\mu_w,(x^*)^T\Sigma_wx^*) \\
P(y^*|Data,x^*)=N((x^*)^T\mu_w,(x^*)^T\Sigma_wx^*+\sigma^2)$ 

但如果遇到的模型是非线性的，一般的做法可以是先做一个非线性转换 $z=\phi(x),\;\;x\in\R^p,\;\;z\in\R^q$，然后再使用贝叶斯线性回归（该非线性转换一般把数据由低维转换成高维）

## 2.1 Process
考虑不存在噪音的情况：
$$f(x^*)|X,Y,x^*\sim N(x^*(\sigma^{-2}A^{-1}X^TY),(x^*)^TA^{-1}x^*)$$
where, $A=\sigma^{-2}X^TX+\Sigma_P^{-1}$

$\text{If }\phi:x\mapsto z,\;\;x\in\R^p,\;\;z=\phi(x)\in\R^q,\;\;q>p \\[3pt]
\text{Define }\Phi=\phi(X)=[(\phi(x_1),...,\phi(x_N))^T]_{N\times q},\;\;X=[(x_1,...,x_N)^T]_{N\times p},\;\;Y=(y_1,...,y_N)^T \\[3pt]
\text{then }f(x)=\phi(x)^Tw$, let $\phi_*$  denotes $\phi(x^*)$
$$f(x^*)|X,Y,x^*\sim N(\phi_*(\sigma^{-2}A^{-1}\Phi^TY),\phi_*^TA^{-1}\phi_*)$$
...
可见它可以被简化成**两数内积**的形式。
此时若进一步观察均值和协方差的表达式，可以发现除了 $Y$ 以及 $\sigma^2I$，其余的所有部分都可以用 kernel function $\kappa(x,x')$ 来表达

**由此可得，该 kernel 已经包含了原特征空间的所有信息，从而成为了原先方法的一种简单替代。GP的过程中 kernel function 的重要性就体现于：它能将一个复杂问题转化成一个只需要考虑 kernel 的简单问题**
...
如果对于任意 $\;\forall\; n\in \N^+$, 都存在映射 $t_1,...,t_n\to \zeta_1,...,\zeta_n$ such that $\bm{\zeta}=(\zeta_1,...,\zeta_n)^T \sim N(\bm{\mu},\bm{\Sigma})$, 那么就把 $\{\zeta_t\}_{t\in T}$ 记为高斯过程, 即
...
\Sigma=\begin{pmatrix}
   ...
   \Sigma_{ba} & \Sigma_{bb}
\end{pmatrix} \\
\text{then we have }X_b|X_a\sim N(\mu_{b|a},\Sigma_{b|a})$$
where,
1. $\mu_{b|a}=\Sigma_{ba}\Sigma_{aa}^{-1}(X_a-\mu_a)+\mu_b$
2. $\Sigma_{b|a}=\Sigma_{bb}-\Sigma_{ba}\Sigma_{aa}^{-1}\Sigma_{ab}$

根据该公式可得
$$P(f(X^*)|Y,X,X^*)=P(f(X^*)|Y)\sim N(\mu^*,\Sigma^*)$$
where,
1. $\mu^*=\kappa(X^*,X)(\kappa(X,X)+\sigma^2I)^{-1}(Y-\mu(X))+\mu(X^*)$
2. $\Sigma^*=\kappa(X^*,X^*)-\kappa(X^*,X)(\kappa(X,X)+\sigma^2I)^{-1}\kappa(X,X^*)$

因此，
$$P(y^*|Y,X,X^*)\sim N(\mu^*,\Sigma^*+\sigma^2I)$$
# 5 Example in code
**简单高斯过程回归实现**
