...
**Variance:** $V[X]=E[(X-\mu_X)^2]=\begin{cases}\sum_{\forall x}(x-\mu_X)^2p(x) \\ \int_{-\infty}^\infty (x-\mu_X)^2f(x)dx\end{cases}$

**Standard Deviation (std):** $\sigma=\sqrt{V[X]}$

**Coefficient of Variance (c.o.v.):** $\delta=\frac{\sigma}{\mu}$
...
(3) Shape of $f(x)$ could be roughly known through $E[(x-\mu_X)^3]$: 
$>0$ means it has a tail to rightside
$=0$ symmetric
$<0$ a tail to leftside
...
$$P(X=x)=\begin{pmatrix}M \\ x\end{pmatrix}p^x(1-p)^{M-x}$$

where $\begin{pmatrix}M \\ x\end{pmatrix}=C_M^x=C(M,x)=\frac{M!}{x!(M-x)!}$
...
(4) $\delta=\frac{\sigma}{\mu} = \sqrt{\frac{1-p}{Mp}}$

(5) $E[(x-\mu_X)^3]=Mp(2p^3-3p+1)$

(6) $\gamma=\frac{E[(x-\mu_X)^3]}{\sigma^3}=\frac{1-2p}{\sqrt{Mp(1-p)}}$
...
Gamma 分布 = 多个独立且相同分布（idd）的指数分布变量的和的分布
