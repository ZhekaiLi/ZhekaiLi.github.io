---
layout: post
title: Math415 Week-02 Appendix
categories: Math415
...
例如对于变换矩阵 $T_{n\times n}$，$T_{n\times n}$ 满秩则意味着该矩阵能将一个在 $\Complex^n$ 中的原图形变换成同样在 $\Complex^n$ 中的一个新图形

**2. 矩阵不满秩即线性相关**
例如对于变换矩阵 $T_{n\times n}$，若 $\text{rank}(T)=n-1$，则意味着该矩阵进行的是从 $\Complex^n$ 至 $\Complex^{n-1}$ 的线性变换
