...
then it's easy to get $|A-\lambda I|=|B-\lambda I|$ 
...
**Corollary 3**: If $A\in\Complex^{n\times n}$ has eigenvalues $\lambda_1,...,\lambda_n$, then $p(A)$ has eigenvalues $p(\lambda_1),...,p(\lambda_n)$ for any polynomial $p$.
...
0 & 0 & 2 & 0 \\
0 & 0 & 0 & 2 
\end{bmatrix}$，$\lambda=2$ 的几何重根等于 $1+2=3$
