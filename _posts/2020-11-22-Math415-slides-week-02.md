...
$$\text{Let }A\in\Complex^{n\times n},\;A^{-1}\text{ is the inverse of }A\text{ if }AA^{-1}=I$$
### 3.2.1 Properties
1. If $A$ has inverse $A^{-1}$, then it is unique
2. $(AB)^{-1}=B^{-1}A^{-1}$
3. $(A^T)^{-1}=(A^{-1})^T$
4. $\text{Let }A\in\Complex^{n\times n},\;x\in\Complex^n,\;\text{if there } \exist\;A^{-1},\;s.t.\;Ax=0 \newline
\text{Then }x=0$
5. $\text{If } \exist\;x\in\Complex^n,\;x\neq 0\;\text{and let }Ax=0 \newline
\text{Then }A \text{ cannot have inverse (is singular)}$


这些概念中，4、5 两点可以从线性变换矩阵的角度来理解（详见文件 week2_appendix.md, Section 2.3.1）

## 3.3 Calculation of inverse
Consider the problem
$$AX=I$$
which is a matrix system with $n^2$ unknowns and $n^2$ equations

Use Gauss Elimination to solve in such way
$$(A|I)\to (I|A^{-1})$$
which is called the augmented matrix
...
当然，对于 $2\times 2$ 矩阵 $A=\begin{pmatrix}
   a & b\\
   c & d
\end{pmatrix}$，可以直接写出逆矩阵 $A^{-1}=\frac{1}{ad-bc}\begin{pmatrix}
   d & -b\\
   c & a
\end{pmatrix}$
...
**Examples**
$P_{12}A$ means change the first and the second row of $A$

## 3.5 Elimination matrix
$\text{Let } A\in\Complex^{n\times n}\text{ be given}$
...
\end{bmatrix}\to E_{21}A=\begin{bmatrix}
   3 & 1 \\
   0 & -\frac{1}{3}
\end{bmatrix}$, then $E_{21}=\begin{bmatrix}
   1 & 0 \\
   -\frac{1}{3} & 1
\end{bmatrix}$
...
并不是所有矩阵都能被 LU 分解，例如 $\begin{bmatrix}
   ...
   1 & 1
\end{bmatrix}$
但是可以通过构造一个 Permutaion Matrix $P$ 使得任何 $A\in\Complex^{n\times n}$ 都能被 LU 分解

### 3.7.2 Find P, L, U from A
from $A\to U$
$U=E_{n(n-1)}E_{n(n-2)}E_{(n-1)(n-2)}...E_{32}E_{n1}...E_{31}E_{21}A=EA$
then
$A=E^{-1}U=LU$
therefore
$L=E^{-1}$
...
**1. $A\in\Complex^{n\times n}$ is invertible $\iff$$\forall\;b\in\Complex^n,\exist\;x\in\Complex^n$ satisfy $Ax=b$**
**证明**:
左到右好证（$x=A^{-1}b$）
